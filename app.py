...
def delayed_delete(file_path, delay=30):
    def delete():
        time.sleep(delay)
...
def home():
    if request.method == "POST":
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            filename=f"downloads/{unique_id}.{info['ext']}"
            os.rename(filename,f"downloads/{unique_id}.mp3")
            filename=f"downloads/{unique_id}.mp3"
            print("################",filename)
      ...
      #  return redirect(url_for("download_file", filename=filename))
        ...
        return redirect(url_for("thank_you", filename=f"{unique_id}.mp3"))
...
def download_file(filename):
    ...
    print("before deleting file name is",filename)
